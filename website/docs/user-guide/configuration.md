...
compression:
  ...
  timing: before_next_turn                          # When threshold compaction runs: before_next_turn | after_reply — see below
...
`timing` chooses **when** the threshold compaction runs; the trigger, protections, and anti-thrash guards are identical either way. `before_next_turn` (default) runs it at the start of your next message, so a conversation you never continue costs nothing extra — but that next reply waits on the summarizer first. `after_reply` runs it right after each completed reply instead, so the next reply starts immediately on an already-compacted transcript; the trade-off is one extra summary pass per threshold crossing even when no further message ever comes. The eager pass never fires for interrupted or failed turns, never for subagents, and if it is skipped or fails the next turn's pre-flight check simply takes over. Same dropdown in Desktop → Settings → Memory & Context.

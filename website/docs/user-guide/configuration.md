...
`timing` chooses **when** the threshold compaction runs (same trigger and guards either way). `before_next_turn` (default) runs it when you send your next message — free if you never do, but that reply waits on the summarizer. `after_reply` runs it right after each reply so the next one starts immediately; costs one summary pass per crossing even if you never continue. Also in Desktop → Settings → Memory & Context.

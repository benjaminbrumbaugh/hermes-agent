...
def _prune_stale_reasoning_replay(messages: List[Dict[str, Any]]) -> int:
    ...
    Only the newest ``type: "compaction"`` carrier can reach the wire; older carriers are shadowed by
    ``native_compaction.prune_pre_checkpoint_items``. Filter items on that carrier instead of popping its
    key. In place; returns pruned message count."""

...
class PromptSubmitParams(SessionParams):
    ...
    # External (non-renderer) desktop turns: keep the renderer's live transport as the session
    # owner and tag the turn so its ``message.start`` / ``message.complete`` can be correlated.
    preserve_session_transport: bool | None = None
...
class PromptSubmitResult(Result):
    ...
    external_submission_id: str | None = None  # echoed from the params for external turns

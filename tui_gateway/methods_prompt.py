...
def _(rid, params: dict) -> dict:
    ...
    # An external controller's submit is not the renderer's surface: leave the owner's value alone.
    ...
    # Re-bind ordinary prompts to the current transport so streaming stays on the active websocket
    # even if a disconnect/fallback moved the session to stdio. Through _rebind_live_transport so a
    # socket that already closed cannot cancel the orphan reap without coming back (#116464).
    # External controllers must retain the live session owner.
    with _session_resume_lock:
        ...
        if preserve_session_transport:
            ...
            if request_transport is not None:
                _rebind_live_transport(sid, session, request_transport)

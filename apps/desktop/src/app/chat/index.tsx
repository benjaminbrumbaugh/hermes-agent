...
export function ChatRuntimeBoundary({
  ...
  // A timeline target that is loaded in the store but carries no durable row
  // cannot be paged in through the around route; grow the runtime window
  // until the message is materialized instead. The target is cleared once it
  // lands, when it leaves the store, or when a newer intent replaces it.
  ...
  useEffect(() => {
    ...
    if (!messages.some(message => message.id === target) || windowedMessages.some(message => message.id === target)) {
      setRevealTarget(null)

...
export interface PromptSubmitParams {
  ...
  preserve_session_transport?: boolean | null
...
export interface RpcMethods {
  ...
  /** Whole-session branch of a STORED parent: session.create with the parent's history copied server-side and the transcript omitted from the reply. A distinct method so older clients can detect the capability. */
  'session.branch_stored': { params: SessionCreateParams; result: SessionCreateResult }
  /** Fork the WHOLE live session (``count`` ignored) without echoing the copied transcript back. */
  'session.branch_whole': { params: SessionBranchParams; result: SessionBranchResult }
...
export interface BackendGatewayEventMap {
  ...
  /** A turn began streaming; payload only identifies an external submission. */
